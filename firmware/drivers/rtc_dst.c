                             ...
                             uint8_t hour)
{
    ...
    rtc_dst_state =
        (rtc_dst_isDateInDST(month, day, hour)) ? RTC_DST_STATE_DST :
        RTC_DST_STATE_ST;
}
...
uint8_t rtc_dst_isDateInDST(uint8_t month, uint8_t day, uint8_t hour)
{
    ...
    if (rtc_dst_dates[0].month < rtc_dst_dates[1].month) {
        /* Northern hemisphere */
        return
            ((DSTNUM(month, day, hour) >=
              DSTNUM(rtc_dst_dates[0].month, rtc_dst_dates[0].day, 2))
             && (DSTNUM(month, day, hour) <
                 DSTNUM(rtc_dst_dates[1].month, rtc_dst_dates[1].day, 2)));
    } else {
        /* Southern hemisphere */
        return (!(((DSTNUM(month, day, hour) >=
                    DSTNUM(rtc_dst_dates[1].month, rtc_dst_dates[1].day, 2))
                   && (DSTNUM(month, day, hour) <
                       DSTNUM(rtc_dst_dates[0].month, rtc_dst_dates[0].day,
                              2)))));
    }
}
